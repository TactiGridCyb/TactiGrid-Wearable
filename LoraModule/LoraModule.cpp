...
int16_t LoraModule::setup(bool transmissionMode_)
{
  ...
  Serial.println(this->lastFrequencyCheck);
...
void LoraModule::onLoraFileDataReceived(const uint8_t* pkt, size_t len)
{
    ...
    if (memcmp(pkt, kFileEndTag, strlen(kFileEndTag)) == 0) 
    {
      ...
      if (this->onFileReceived && this->expectedChunks > 0 && this->receivedChunks == this->expectedChunks) 
      {
...
float LoraModule::setFrequency(float newFreq)
{
  this->freq = newFreq;
  loraDevice.setFrequency(freq);
  return freq;
