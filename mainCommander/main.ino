    ...
    std::unique_ptr<Commander> commandersModule)
{
    ...
    std::move(wifiModule), newGPSModule, std::move(newFHFModule), std::move(commandersModule), logFilePath, true, true);
...
void transferFromMainToReceiveCoordsPage()
{
    ...
    commandersMissionPage = std::make_unique<CommandersMissionPage>(loraModule,
         std::move(wifiModule), gpsModule, std::move(fhfModule), std::move(commandersModule), logFilePath, false, true);
    commandersMissionPage->createPage();
