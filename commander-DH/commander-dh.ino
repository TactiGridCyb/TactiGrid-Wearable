...
// bool decodeBase64(const String& input, std::vector<uint8_t>& output) {
//   size_t decodedLen = (input.length() * 3) / 4 + 2;
//   output.resize(decodedLen);
//   size_t outLen = 0;
//   int ret = mbedtls_base64_decode(output.data(), output.size(), &outLen,
...
//   //String certB64 = toBase64(std::vector<uint8_t>(config->getCertificatePEM().begin(), config->getCertificatePEM().end()));
...
// void onLoRaData(const uint8_t* data, size_t len) {
//   if (!waitingResponse) {
//     Serial.println("⚠️ Ignoring extra response");
//     return;
//   }
